...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place, dis = self.place, 0
        while not place.is_hive:
            bee = random_bee(place.bees)
            if bee and self.min_range <= dis <= self.max_range:
                return bee
...
class HungryAnt(Ant):
    ...
    def action(self, gamstate):
        if self.chew_countdown == 0 and self.place.bees[:]:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
        if not cls.is_construct:
            cls.is_construct = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant != self:
            front_insects[self.place.ant] = 0
        place = self.place
        distance = 0
        while place != None and not place.is_hive:
            for bee in place.bees:
                front_insects[bee] = distance
            if place.ant and place.ant != self:
                front_insects[place.ant] = distance
                if place.ant.is_container and place.ant.ant_contained and place.ant.ant_contained != self:
                    front_insects[place.ant.ant_contained] = distance
            place = place.entrance
            distance += 1
